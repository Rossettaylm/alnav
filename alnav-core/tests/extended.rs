...
use alnav::crash::{CrashDetector, CrashType};
use alnav::dedupe::{Deduper, Normalizer};
use alnav::expr::Expr;
use alnav::filter::FilterChain;
use alnav::formatter::{FieldSet, Formatter};
use alnav::histogram::{parse_interval, Histogram};
use alnav::multiline::MultilineMerger;
use alnav::parser::{Level, LogEntry};
use alnav::sampler::Sampler;
...
fn test_expr_deeply_nested() {
    ...
    let expr_str =
        "((tag ~ OkHttp or tag ~ Retrofit) and (msg ~ timeout or msg ~ refused)) and level >= W";
...
fn test_multiline_single_entry_no_continuation() {
    let input = ok_lines(&["04-02 12:34:56.789  1234  5678 D Tag     : single line"]);
    let merged: Vec<String> = MultilineMerger::new(input.into_iter())
        .map(|r| r.unwrap())
        .collect();
