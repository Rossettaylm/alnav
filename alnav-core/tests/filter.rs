use alnav::filter::FilterChain;
use alnav::parser::{Level, LogEntry};
use alnav::{Cli, OutputFormat};
...
fn test_cross_type_and_logic() {
    ...
    assert!(
        chain.matches(&LogEntry::parse(&make_entry(Level::D, "OkHttp", "error occurred")).unwrap())
    );
    assert!(!chain.matches(&LogEntry::parse(&make_entry(Level::D, "OkHttp", "success")).unwrap()));
    assert!(
        !chain.matches(&LogEntry::parse(&make_entry(Level::D, "MyApp", "error occurred")).unwrap())
    );
}
...
fn test_msg_and_logic() {
    ...
    assert!(chain.matches(
        &LogEntry::parse(&make_entry(Level::I, "NT", "mobile_msf cmd:0x9293 done")).unwrap()
    ));
    assert!(!chain.matches(
        &LogEntry::parse(&make_entry(Level::I, "NT", "mobile_msf cmd:0xfe1 done")).unwrap()
    ));
    assert!(!chain
        .matches(&LogEntry::parse(&make_entry(Level::I, "NT", "other cmd:0x9293 done")).unwrap()));
}
...
fn test_time_full_datetime_xlog() {
    let chain = build_chain_time(Some("2026-03-04 10:30:00"), Some("2026-03-04 10:35:00"));
...
fn test_time_full_date_threadtime() {
    let chain = build_chain_time(Some("04-02 12:00:00"), Some("04-02 13:00:00"));
    let hit = "04-02 12:34:56.789  1234  5678 D Tag     : msg";
...
fn test_time_full_xlog_cross_day() {
    let chain = build_chain_time(Some("2026-03-03 23:00:00"), Some("2026-03-04 01:00:00"));
