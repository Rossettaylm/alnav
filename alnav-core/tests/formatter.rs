...
fn test_context_line_json_parseable() {
    ...
    let mut buf = Vec::new();
    f.write_context_line("04-02 10:00:00.000  1234  5678 D Tag     : hello", &mut buf)
        .unwrap();
    let out = String::from_utf8(buf).unwrap();
...
fn test_context_line_json_unparseable() {
    let f = make_formatter(OutputFormat::Json);
    let mut buf = Vec::new();
    f.write_context_line("some random unparseable line", &mut buf)
        .unwrap();
    let out = String::from_utf8(buf).unwrap();
    assert!(out.contains("\"context\":true"));
...
fn test_context_line_json_escaping() {
    let f = make_formatter(OutputFormat::Json);
    let mut buf = Vec::new();
    f.write_context_line("line with \"quotes\" and \\backslash", &mut buf)
        .unwrap();
    let out = String::from_utf8(buf).unwrap();
