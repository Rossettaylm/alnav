...
impl Sampler {
    ...
    pub fn new(tail: usize, sample: usize, limit: usize) -> Self {
        ...
        Self {
            ...
            reservoir: if sample > 0 {
                Vec::with_capacity(sample)
            } else {
                Vec::new()
            },
            rng: 42,
    ...
    pub fn finish(mut self) -> SamplerResult {
        match self.mode {
            SampleMode::PassThrough => SamplerResult {
                lines: Vec::new(),
                header: None,
            },
            SampleMode::HeadTail { head, .. } => {
                ...
                let header = if skipped > 0 {
                    if head > 0 {
                        Some(format!(
                            "{skipped} entries omitted ({} total matched)",
                            self.total
                        ))
                    } else {
                        Some(format!(
                            "showing last {tail_count} of {} matched entries",
                            self.total
                        ))
                    }
                } else {
