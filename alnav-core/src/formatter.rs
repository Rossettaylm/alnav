    ...
    pub fn parse(input: &str) -> Result<Self, String> {
        ...
        for field in input.split(',') {
            ...
            match f.as_str() {
                ...
                _ => {
                    return Err(format!(
                        "unknown field '{}', expected: timestamp,pid,tid,level,tag,pkg,msg",
                        f
                    ))
                }
            }
...
impl Formatter {
        ...
        let user_highlights = if use_color {
            highlight_args
                ...
                .filter_map(|(i, pat)| {
                    ...
                    regex::Regex::new(&re_pat)
                        .ok()
                        .map(|re| (re, i % logcolor::USER_HIGHLIGHT.len()))
                })
                .collect()
        } else {
            vec![]
    ...
    fn write_text<W: Write>(
        ...
        let pkg_str = if !entry.pkg.is_empty() {
            format!(
                ...
                entry
                    .pkg
                    .truecolor(logcolor::PKG.0, logcolor::PKG.1, logcolor::PKG.2)
            )
        } else {
            String::new()
        };
    ...
    fn write_json<W: Write>(&self, entry: &LogEntry, out: &mut W) -> std::io::Result<()> {
        ...
        if f.timestamp {
            json_field!("timestamp", entry.timestamp.trim());
        }
        if f.pid {
            json_field!("pid", entry.pid);
        }
        if f.tid {
            json_field!("tid", entry.tid);
        }
        if f.level {
            json_field!("level", entry.level.as_char());
        }
        if f.tag {
            json_field_escaped!("tag", entry.tag);
        }
        if f.pkg && !entry.pkg.is_empty() {
            json_field_escaped!("pkg", entry.pkg);
        }
        if f.msg {
            json_field_escaped!("msg", entry.msg);
        }
        let _ = first;
        writeln!(out, "}}")
    }
    ...
    fn write_csv<W: Write>(&self, entry: &LogEntry, out: &mut W) -> std::io::Result<()> {
        ...
        if f.pid {
            parts.push(entry.pid.to_string());
        }
        if f.tid {
            parts.push(entry.tid.to_string());
        }
        if f.level {
            parts.push(entry.level.as_char().to_string());
        }
        if f.tag {
            parts.push(entry.tag.to_string());
        }
        if f.pkg && !entry.pkg.is_empty() {
            parts.push(entry.pkg.to_string());
        }
        if f.msg {
        ...
        for re in &self.highlight_patterns {
            let replaced = re.replace_all(&result, |caps: &regex::Captures| {
                caps[0]
                    .bold()
                    .on_truecolor(
                        logcolor::FILTER_MATCH.0,
                        logcolor::FILTER_MATCH.1,
                        logcolor::FILTER_MATCH.2,
                    )
                    .to_string()
            });
    ...
    fn write_text_fields<W: Write>(&self, entry: &LogEntry, out: &mut W) -> std::io::Result<()> {
        ...
        if f.timestamp && !entry.timestamp.is_empty() {
            parts.push(entry.timestamp.trim());
        }
        ...
        if f.level {
            level_str = entry.level.as_char().to_string();
    ...
    fn write_text_fields_color<W: Write>(
        &self,
        entry: &LogEntry,
        out: &mut W,
    ) -> std::io::Result<()> {
    ...
    pub fn write_dedupe_group<W: Write>(
        &self,
        group: &DedupGroup,
        out: &mut W,
    ) -> std::io::Result<()> {
